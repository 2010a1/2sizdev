...
const TrueFalseStatementSchema = z.object({ id:z.string().min(1), text:z.string().min(1), correct:z.boolean() }).strict();
export const TrueFalseQuestionSchema = z.object({ ...BaseQuestionFields, type:z.literal("TRUE_FALSE"), correctAnswer:z.boolean().optional(), statements:z.array(TrueFalseStatementSchema).length(4).optional() }).strict().superRefine((q,ctx)=>{
  if(!q.statements && typeof q.correctAnswer !== "boolean") ctx.addIssue({code:"custom",path:["correctAnswer"],message:"TRUE_FALSE phải có correctAnswer hoặc 4 statements"});
  if(q.statements && typeof q.correctAnswer !== "boolean" && q.statements.length!==4) ctx.addIssue({code:"custom",path:["statements"],message:"TRUE_FALSE phải có đúng 4 mệnh đề"});
});
...
export const ExamDraftQuestionSchema = z.discriminatedUnion("type", [
  ...
  // Single-statement TRUE_FALSE (correctAnswer boolean) was removed from
  // creation: the true/false form must carry 4 statements. ExamContentSchema
  // below still accepts the legacy single form so old .exam packages import.
  z.object({...DraftBase,type:z.literal("TRUE_FALSE"),statements:z.array(TrueFalseStatementSchema).length(4)}).strict(),
...
export const ShareCreateRequestSchema = z.object({ packageBase64:z.string().min(1), contentHash:z.string().regex(/^sha256:[0-9a-f]{64}$/), formatVersion:z.number().int().nonnegative(), expiresIn:z.enum(['24h','7d','never']).default('7d'), ownerDeviceId:z.string().max(200).optional() });
