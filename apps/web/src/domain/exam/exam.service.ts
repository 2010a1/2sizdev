...
export const examService = {
  async createExamFromJson(input: unknown): Promise<Exam> {
    ...
    const rawQuestions = draft.questions.map((q, order) => {
      const base:any={id:q.id,examId:id,order,content:q.content,points:q.points??1,explanation:q.explanation,imageAssetId:(q as any).imageAssetId,imageUrl:(q as any).imageUrl};
      if(q.type==='ABCD') return {...base,type:'ABCD',options:q.options,correctOptionId:q.correctOptionId};
      if(q.type==='TRUE_FALSE') return {...base,type:'TRUE_FALSE',correctAnswer:q.statements.every(s=>s.correct),statements:q.statements};
    ...
    await localMutationService.exam(exam.id,'CREATE');
    return exam;
  },
  async createExam(input: {title:string; description?:string; subject:string; grade?:number; duration?:number}): Promise<Exam> {
    ...
    await examRepository.createExam(exam); await localMutationService.exam(exam.id, 'CREATE'); return exam;
  },
  ...
  async updateExam(id:string, patch:Partial<Exam>) { const exam=await this.getExam(id); if(exam.source!=='local') throw new ExamDomainError('Đề này chỉ được xem. Hãy sao chép để chỉnh sửa.','READ_ONLY'); const normalized={...patch, ...(patch.duration !== undefined ? { duration: validateDurationSeconds(patch.duration) } : {}), updatedAt:nowTs()}; await examRepository.updateExam(id,normalized); await localMutationService.exam(id, 'UPDATE'); return this.getExam(id); },
  async deleteExam(id:string) {
    ...
    await localMutationService.exam(id, 'DELETE');
    for (const q of removed.questions) await localMutationService.question(q.id, 'DELETE', undefined, q);
    for (const asset of removed.assets) await localMutationService.enqueue(undefined, 'examAsset', asset.id, 'DELETE', { id: asset.id, examId: id, path: asset.path, hash: asset.hash }, Date.now(), (asset as any).syncRevision ?? 0);
  },
  async duplicateExam(id:string) {
    const source=await this.getExam(id);
    const questions=await examRepository.getQuestionsByExam(id);
    const now=nowTs();
    const newId=generateId('exam');
    // A copy is a fresh local exam: it must not inherit the source's favourite flag
    // nor its sync provenance (syncRevision/syncOrigin belong to the source's server row).
    const exam={...source,id:newId,title:`${source.title} (Bản sao)`,source:'local',version:1,contentHash:'local',createdAt:now,updatedAt:now,deletedAt:undefined,questionCount:questions.length,isFavorite:false,syncRevision:0,syncOrigin:undefined} as unknown as Exam;
    // Questions may reference image assets stored under the *source* exam's id
    // (examAssets is keyed by examId). Those rows must be cloned under newId too,
    // otherwise: (a) exporting/sharing the copy can't find its own assets (it only
    // scans examAssets for its own examId), and (b) deleting the original exam later
    // cascades and deletes assets the copy still silently depends on.
    const sourceAssets = await examRepository.getAssetsByExam(id);
    const assetIdMap = new Map<string,string>();
    const duplicatedAssets = sourceAssets.map(asset => {
      const newAssetId = generateId('asset');
      assetIdMap.set(asset.id, newAssetId);
      return { ...asset, id: newAssetId, examId: newId };
    });
    const newQuestions = questions.map(q => ({
      ...q,
      id: generateId('question'),
      examId: newId,
      imageAssetId: q.imageAssetId ? assetIdMap.get(q.imageAssetId) : undefined
    }));
    await examRepository.duplicateExam(newId, exam, newQuestions, duplicatedAssets);
    await localMutationService.exam(newId, 'CREATE');
    return exam;
  },
  async favoriteExam(id:string) { const exam=await this.getExam(id); await examRepository.favoriteExam(id,!exam.isFavorite); await localMutationService.exam(id, 'UPDATE'); return !exam.isFavorite; },
  async refreshQuestionCount(id:string) { const questions=await examRepository.getQuestionsByExam(id); await examRepository.updateExam(id,{questionCount:questions.length,updatedAt:nowTs()}); await localMutationService.exam(id, 'UPDATE'); return questions.length; }
};
