...
export function applyCompetitionOrder(questions:Question[],questionOrder:string[],optionOrderByQuestion:Record<string,string[]>):Question[]{
  ...
  const savedIds=questionOrder.filter(id=>byId.has(id)&&!savedSeen.has(id)&&!!savedSeen.add(id));
  const seen=new Set<string>();
  const orderedIds: string[] = [];
  for (const section of EXAM_SECTION_ORDER) {
    for (const id of savedIds) { const q=byId.get(id)!; if(sectionForQuestion(q)===section) { orderedIds.push(id); seen.add(id); } }
