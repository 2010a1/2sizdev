...
export interface TournamentEngineOptions {
  ...
  visitedQuestions?: string[]; flaggedQuestions?: string[];
  correctCount?: number; wrongCount?: number; streak?: number; bestStreak?: number;
  status?: TournamentState["status"]; finishedAt?: number; questionEnteredAt?: number; now?: number;
}
...
export class TournamentEngine {
  ...
  getState(now = Date.now()): TournamentState {
    return { ...this.state, answers: { ...this.state.answers }, visitedQuestions: [...this.state.visitedQuestions], flaggedQuestions: [...this.state.flaggedQuestions], elapsedTime: this.elapsed(now) };
  }
