...
async function sha256(bytes: Uint8Array) { const hash = await crypto.subtle.digest("SHA-256", bytes); return `sha256:${Array.from(new Uint8Array(hash)).map(b => b.toString(16).padStart(2, "0")).join("")}`; }
...
export async function createVocabularyShare(profileId: string, setId: string, expiresIn: "24h" | "7d" | "never" = "7d") {
  ...
  const r = await fetch(apiUrl("/api/share"), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ packageType: "vocabularySet", packageBase64: base64(bytes), contentHash, formatVersion: 1, expiresIn }) });
