...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    const response = await fetch(apiUrl(path), { ...init, signal: controller.signal, headers: { 'content-type': 'application/json', ...(init?.headers ?? {}) } });
...
export interface ShareResponse { shareId:string; shareCode:string; packageBase64?:string; packageType:'exam'|'vocabularySet'; contentHash:string; formatVersion:number; storageKey?:string; createdAt:number; expiresAt?:number; shareUrl:string; }
...
export async function createExamShare(examId:string, expiresIn:'24h'|'7d'|'never'='7d') {
  ...
  return request<ShareResponse>('/api/share', { method:'POST', body:JSON.stringify({ packageType:'exam', packageBase64:base64(bytes), contentHash:imported.contentHash, formatVersion:imported.formatVersion, expiresIn }) });
}

export async function getShare(code:string) {
  const normalized = code.trim().toUpperCase();
  if (!/^[A-HJ-NP-Z2-9]{6,10}$/.test(normalized)) throw new Error('Mã đề không hợp lệ.');
...
export async function importExamShare(code:string) {
  ...
  await db.sharedExams.put({ id:share.shareId, examId:exam.id, code:share.shareCode, expiresAt:share.expiresAt, importedAt:Date.now() });
  return exam;
}
