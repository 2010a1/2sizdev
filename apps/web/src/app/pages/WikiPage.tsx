...
export function WikiPage() {
  ...
  return <div className="wiki-layout">
    <aside className="wiki-sidebar">
      ...
      <nav className="wiki-nav">{visible.map((s) => <button key={s.id} className={current.id === s.id ? "active" : ""} onClick={() => setActive(s.id)}><AppIcon name={s.icon} size={17}/><span>{s.title}</span></button>)}</nav>
    ...
    <main className="wiki-main">
      ...
      <article className="wiki-article" key={current.id}>
        <div className="wiki-title-row"><div className="wiki-title-icon"><AppIcon name={current.icon} size={25}/></div><div><span className="eyebrow">CHỦ ĐỀ</span><h2>{current.title}</h2><p>{current.summary}</p></div></div>
