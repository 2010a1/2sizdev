...
export function ProfileGate({ children }: { children: React.ReactNode }) {
  ...
  if(authLoading||loading||bootstrapping)return <div className="min-h-screen grid place-items-center muted text-sm">Đang khởi tạo tài khoản...</div>;
  if(!authUser)return <Navigate to="/login" replace />;
  if(!activeProfile)return <div className="min-h-screen grid place-items-center muted text-sm">Đang tạo hồ sơ học tập...</div>;
  return <>{children}</>;
}
