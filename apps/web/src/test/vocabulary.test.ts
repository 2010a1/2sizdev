...
describe("deterministic generator",()=>{
 it("generates exactly four types deterministically",()=>{
  ...
  const mc=a.find(q=>q.type==="MC_EN_TO_VI")!; expect(mc.availability).toBe("available"); expect(mc.options).toHaveLength(4); expect(new Set(mc.options)).toHaveSize(4); expect(mc.options?.filter(x=>x===mc.answer)).toHaveLength(1);
 ...
 it("handles one-letter and repeated-letter ordering",()=>{for(const w of ["A","AAAA","BOOK"]){const q=generateVocabularyQuestions(word("v"+w,w,w),[word("v"+w,w,w)],1).find(q=>q.type==="LETTER_ORDER")!;expect(q.letters?.join("")).toBe(w);expect([...q.letters??[]].sort()).toEqual([...w].sort());}});
});
...
describe("practice persistence/recovery",()=>{
 it("persists deterministic session order, answer, index and flags across reconstruction",async()=>{
  ...
  const s3=await vocabularySessionService.create("p1",v.id,"practice",4); const answer=s3.questions[0].type==="MC_EN_TO_VI"?"":s3.questions[0].answer; await vocabularySessionService.answer("p1",s3.session.id,answer,5_000); const saved=await vocabularyRepository.getSessionAnswers(s3.session.id); expect(saved[0].timeSpent).toBe(5); await vocabularySessionService.next("p1",s3.session.id); await vocabularySessionService.flag("p1",s3.session.id);
