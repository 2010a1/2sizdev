...
describe('Phase 7 sync',()=>{
  ...
  it('skips a poison record, keeps the good ones and advances the cursor',async()=>{
    ...
    // Old behaviour (throw on the first invalid record) wedged sync forever: the
    // cursor never advanced so every later sync re-pulled the same poison batch.
    expect(await syncEngine.sync('p1',transport)).toBe('IDLE'); expect((await db.exams.get('e3'))?.title).toBe('ok'); expect(await db.questions.get('q1')).toBeUndefined(); expect((await db.syncState.get('profile:p1'))?.cursor ?? 0).toBe(2);
  });
});
