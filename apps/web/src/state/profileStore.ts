...
export const useProfileStore = create<ProfileStoreState>((set, get) => ({
  ...
  async refresh() {
    ...
    // Re-read the active profile from the fresh list — keeping the old
    // object would show stale avatar/name until the next reload.
    const fresh = current && profiles.find((p) => p.id === current.id);
    const next = fresh ?? profiles[0] ?? null;
