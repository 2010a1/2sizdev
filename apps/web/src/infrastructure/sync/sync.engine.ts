...
export const syncEngine = {
  ...
  async sync(profileId: string, transport: SyncTransport = apiSyncTransport): Promise<SyncStatus> {
    ...
    try {
      ...
      if (accountId) await syncRepository.adoptOrphaned(profileId, accountId);
      ...
      // Spread the previous state so the sync cursor (and deviceId) survive the
      // error. saveState() replace-writes the whole row; dropping the cursor here
      // would force the next sync to replay the entire remote history.
      const failed = await syncRepository.state(profileId, accountId);
      await syncRepository.saveState({ ...failed, key, profileId, accountId, status: 'ERROR', lastError: message });
      return 'ERROR';
