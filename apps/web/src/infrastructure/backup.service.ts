...
async function snapshot():Promise<BackupPayload>{const tables:Record<string,unknown[]>={};for(const name of RESTORE_TABLES) tables[name]=await (db as any).table(name).toArray();return {format:'exam-platform-backup',version:BACKUP_VERSION,exportedAt:Date.now(),tables};}
...
function assertRow(table:string,row:any){
  ...
  if(['exams','questions','attempts','answers','vocabularies','vocabQuestions','vocabProgress','vocabSessions','vocabSessionAnswers','examAssets','vocabularySets','vocabularySetItems'].includes(table)) assertString(row,'profileId',table);
...
export async function restoreLocalBackup(file:File,mode:'merge'|'replace'='merge'){const backup=await parseBackup(file);await db.transaction('rw',...RESTORE_TABLES.map(name=>(db as any).table(name)),async()=>{if(mode==='replace')for(const name of RESTORE_TABLES)await (db as any).table(name).clear();for(const name of RESTORE_TABLES){const rows=backup.tables[name]??[];if(!rows.length)continue;if(mode==='replace'){await (db as any).table(name).bulkPut(rows);continue;}const ids=rows.map((r:any)=>r.id);const existing=new Set<string>();for(let i=0;i<ids.length;i+=1000){const current=await (db as any).table(name).bulkGet(ids.slice(i,i+1000));current.forEach((row:any)=>{if(row?.id)existing.add(row.id)});}const fresh=rows.filter((r:any)=>!existing.has(r.id));if(fresh.length)await (db as any).table(name).bulkPut(fresh);}});return {conflicts:backup.conflicts,totalRows:backup.totalRows};}
