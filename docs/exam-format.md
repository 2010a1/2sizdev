...
`CURRENT_EXAM_FORMAT_VERSION = 1`. The migration API is explicit: `migrateExamContent(content, fromVersion)`. Version 1 is current; future versions are rejected with `UNSUPPORTED_VERSION`. When a future format version is introduced, each transition must be implemented explicitly (for example `migrateV1ToV2`).
